...
export class UsuarioController {
  ...
  register = async (req: Request, res: Response) => {
    ...
    if (domicilioDto){
      const domicilioCreado = await this.domicilioService.create(domicilioDto);

      clienteDto?.setIdDomicilio(domicilioCreado.getID())
    }
...
modificarUsuario = async (req: Request, res: Response): Promise<void> => {
  const userId = req.user?.id;  // Suponiendo que el id del usuario se pasa como parámetro en la URL
